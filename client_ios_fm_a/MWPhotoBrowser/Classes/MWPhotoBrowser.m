...
- (void)_initialisation {
    ...
    _actionAllType = ACTION_ALL_TYPE_DELETE;    //默认为删除
...
//    frame.size.width += (2 * PADDING);
    ...
    frame.origin.x += 5;    //不靠边，防止无法监听到滑动返回事件
...
- (void)showGrid:(BOOL)animated {
    ...
    if(_actionAllType != ACTION_ALL_TYPE_UNKNOW) {
        if(!_actionAllButton && _displayActionButton) {
            _actionAllButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemTrash target:self action:@selector(actionAllButtonPressed:)];
        }
        if(self.navigationItem.rightBarButtonItem) {
            if(self.navigationItem.rightBarButtonItem != _actionAllButton&& self.navigationItem.rightBarButtonItem != _actionButton) {
                _gridPreviousRightNavItem = self.navigationItem.rightBarButtonItem;
                NSMutableArray * menusArray = [[NSMutableArray alloc] init];
                [menusArray addObject:self.navigationItem.rightBarButtonItem];
                if(_actionAllButton) {
                    [menusArray addObject:_actionAllButton];
                }
                self.navigationItem.rightBarButtonItems = menusArray;
            } else {
                self.navigationItem.rightBarButtonItem = _actionAllButton;
            }
        } else {
            self.navigationItem.rightBarButtonItem = _actionAllButton;
        }
...
- (void)hideGrid {
    ...
    if(_actionAllType != ACTION_ALL_TYPE_UNKNOW) {
        if(self.navigationItem.rightBarButtonItem && _gridPreviousRightNavItem) {
            [self.navigationItem setRightBarButtonItem:_gridPreviousRightNavItem animated:YES];
        } else {
            for(UIBarButtonItem * item in self.navigationItem.rightBarButtonItems) {
                if(item && item != _actionAllButton) {
...
- (void)actionAllButtonPressed:(id)sender {
    {
        ...
        if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
            ...
            // If they have defined a delegate method then just message them
            if ([self.delegate respondsToSelector:@selector(photoBrowser:actionButtonAllPressed:)]) {
                // Let delegate handle things
                [self.delegate photoBrowser:self actionButtonAllPressed:_currentPageIndex];
