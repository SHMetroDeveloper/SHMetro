...
@interface DispachOrderViewController ()
...
@property (readwrite, nonatomic, strong) OrderDispatchTableHelper * ordersHelper;
...
@implementation DispachOrderViewController
...
- (void)initNavigation {
    [self setTitleWith:[[BaseBundle getInstance] getStringByKey:@"function_order_dispach" inTable:nil]];
...
- (void)initLayout {
    ...
    _ordersHelper = [[OrderDispatchTableHelper alloc] initWithContext:self];
    ...
    [_noticeLbl setInfoWithName:[[BaseBundle getInstance] getStringByKey:@"order_dispach_no_data" inTable:nil] andLogo:[[FMTheme getInstance] getImageByName:@"no_data"] andHighlightLogo:[[FMTheme getInstance] getImageByName:@"no_data"]];
...
- (void) requestData {
    ...
    [_business getOrdersDispachPage:mpage Success:^(NSInteger key, id object) {
        DispachWorkOrderResponseData * response = object;
...
- (void) showOrderDetail:(WorkOrderDispach *) order {
...
- (void)handleMessage:(id)msg {
    ...
    if ([strOrigin isEqualToString:NSStringFromClass([_ordersHelper class])]) {
        ...
        OrderDispatchEventType type = [[result valueForKeyPath:@"eventType"] integerValue];
        if (type == WO_DISPATCH_EVENT_ITEM_CLICK) {
            WorkOrderDispach * order = [result valueForKeyPath:@"eventData"];
