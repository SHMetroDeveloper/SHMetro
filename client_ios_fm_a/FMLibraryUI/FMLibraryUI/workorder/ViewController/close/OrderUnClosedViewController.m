...
#import "OrderCloseTableHelper.h"
#import "ImageItemView.h"
#import "BaseBundle.h"


@interface OrderUnClosedViewController () <OnMessageHandleListener>
...
@property (readwrite, nonatomic, strong) OrderCloseTableHelper* ordersHelper;
...
@implementation OrderUnClosedViewController
...
- (void) initNavigation {
    [self setTitleWith:[[BaseBundle getInstance] getStringByKey:@"function_order_close" inTable:nil]];
...
- (void) initLayout {
    ...
    _ordersHelper = [[OrderCloseTableHelper alloc] initWithContext:self];
    ...
    [_noticeLbl setInfoWithName:[[BaseBundle getInstance] getStringByKey:@"order_close_no_data" inTable:nil] andLogo:[[FMTheme getInstance] getImageByName:@"no_data"] andHighlightLogo:[[FMTheme getInstance] getImageByName:@"no_data"]];
...
- (void) requestData {
    ...
    [_business getOrdersUnClosedPage:mpage Success:^(NSInteger key, id object) {
        WorkOrderUnClosedResponseData * response = object;
...
- (void) showOrderDetail:(WorkOrderUnClosedEntity *) order {
...
- (void) handleMessage:(id)msg {
    ...
    if([strOrigin isEqualToString:NSStringFromClass([_ordersHelper class])]) {
        ...
        OrderUnCloseEventType type = [[result valueForKeyPath:@"eventType"] integerValue];
        if(type == WO_UNCLOSE_EVENT_ITEM_CLICK) {
            WorkOrderUnClosedEntity * order = [result valueForKeyPath:@"eventData"];
