//
//  WorkOrderGrabedViewController.m
//  client_ios_fm_a
//
...
#import "WorkOrderGrabedViewController.h"
...
@interface WorkOrderGrabedViewController () <UITableViewDataSource, UITableViewDelegate, PullTableViewDelegate, OnItemClickListener>
...
@implementation WorkOrderGrabedViewController
...
- (void) initLayout {
    if(!_mainContainerView) {
        ...
        [_noticeLbl setInfoWithName:[[BaseBundle getInstance] getStringByKey:@"order_grabed_no_data" inTable:nil] andLogo:[[FMTheme getInstance] getImageByName:@"no_data"] andHighlightLogo:[[FMTheme getInstance] getImageByName:@"no_data"]];
...
- (void) initNavigation {
    [self setTitleWith:[[BaseBundle getInstance] getStringByKey:@"function_order_grabed" inTable:nil]];
...
- (void) requestData {
    ...
    [netRequest request:param success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        for(NSDictionary * order in orders) {
            ...
            NSInteger tmp = arc4random() % 3;   //TODO:抢单测试代码
            switch(tmp) {
                case 0:
                    job.grabStatus = WORK_ORDER_GRAB_LABORER_STATUS_WAITING;
                    break;
                case 1:
                    job.grabStatus = WORK_ORDER_GRAB_LABORER_STATUS_SUCCESS;
                    break;
                default:
                    job.grabStatus = WORK_ORDER_GRAB_LABORER_STATUS_FAIL;
                    break;
            }
