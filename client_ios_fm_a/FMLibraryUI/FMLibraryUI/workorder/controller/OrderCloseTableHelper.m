...
@interface OrderCloseTableHelper ()
...
@implementation OrderCloseTableHelper
...
- (WorkOrderUnClosedEntity *) getOrderByPosition:(NSInteger) position {
    WorkOrderUnClosedEntity * order = _orders[position];
    return order;
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    WorkOrderUnClosedEntity* job = _orders[position];
...
- (void) notifyShowOrderDetail:(NSInteger) position {
    WorkOrderUnClosedEntity * workJob = _orders[position];
    [self notifyEvent:WO_UNCLOSE_EVENT_ITEM_CLICK data:workJob];
    
}

- (void) notifyEvent:(OrderUnCloseEventType) type data:(id) data {
    if(_handler) {
