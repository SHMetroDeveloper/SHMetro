//
//  EvaluateRequirementViewController.m
//  client_ios_fm_a
//
...
@interface EvaluateRequirementViewController () <UITableViewDataSource, UITableViewDelegate, PullTableViewDelegate>
...
@implementation EvaluateRequirementViewController
...
- (void) initLayout {
    if(!_mainContainerView) {
        ...
        [_noticeLbl setInfoWithName:[[BaseBundle getInstance] getStringByKey:@"requirement_evaluate_no_data" inTable:nil] andLogo:[[FMTheme getInstance] getImageByName:@"no_data"] andHighlightLogo:[[FMTheme getInstance] getImageByName:@"no_data"]];
...
- (void) initNavigation {
    [self setTitleWith:[[BaseBundle getInstance] getStringByKey:@"function_requirement_evaluate" inTable:nil]];
...
- (void) requestData {
    ...
    RequirementRequestParam * param = [[RequirementRequestParam alloc] initWithPage:_mPage andQueryType:REQUIREMENT_TYPE_DONE andCondition:nil];
    
    [_business getUndoRequirementListDataByParam:param Success:^(NSInteger key, id object) {
...
- (void) updateList {
    ...
    if(!_requirementArray || [_requirementArray count] == 0) {
        [self updateNotice:[[BaseBundle getInstance] getStringByKey:@"requirement_evaluate_no_data" inTable:nil] display:YES];
    } else {
