...
- (id)initWithTitle:(NSString *)title
   ...
   rightButtonTitle:(NSString *)rigthTitle
     viewController:(UIViewController *) containerVC;
{
    if (self = [super init]) {
...
//        [self.leftBtn setBackgroundImage:[UIImage imageWithColor:[UIColor colorWithRed:227.0/255.0 green:100.0/255.0 blue:83.0/255.0 alpha:1]] forState:UIControlStateNormal];
        ...
        [self.rightBtn setBackgroundImage:[UIImage imageWithColor:[[FMTheme getInstance] getColorByResource:FM_RESOURCE_TYPE_COLOR_THEME]] forState:UIControlStateNormal];
        [self.leftBtn setBackgroundImage:[UIImage imageWithColor:[[FMTheme getInstance] getColorByResource:FM_RESOURCE_TYPE_COLOR_MAIN_RED]] forState:UIControlStateNormal];
        ...
        [xButton setImage:[[FMTheme getInstance] getImageByName:@"btn_close_normal"] forState:UIControlStateNormal];
        [xButton setImage:[[FMTheme getInstance] getImageByName:@"btn_close_selected"] forState:UIControlStateHighlighted];
...
- (void)show
{
    _containerVC = [self appRootViewController];
    self.frame = CGRectMake((CGRectGetWidth(_containerVC.view.bounds) - kAlertWidth) * 0.5, - kAlertHeight - 30, kAlertWidth, kAlertHeight);
    [_containerVC.view addSubview:self];
}

- (void)showIn:(UIViewController *) topVC1
{
    _containerVC = [self appRootViewController];
    self.frame = CGRectMake((CGRectGetWidth(_containerVC.view.bounds) - kAlertWidth) * 0.5, - kAlertHeight - 30, kAlertWidth, kAlertHeight);
    [_containerVC.view addSubview:self];
...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
    ...
    UIViewController *topVC = _containerVC;
    if(!topVC ) {
        topVC = [self appRootViewController];
    }
