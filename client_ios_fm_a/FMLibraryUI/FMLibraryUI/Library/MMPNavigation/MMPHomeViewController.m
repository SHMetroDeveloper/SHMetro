...
- (void)moveLeft {
    ...
    if (childView) {
        ...
        [UIView animateWithDuration:ANIMATE_DURATION
                         ...
                         animations:^{
                             _mainVC.view.frame = CGRectMake(-self.view.frame.size.width + SHOW_MAIN_WIDTH, originY, self.view.frame.size.width, CGRectGetHeight(frame));
                         }
...
-(void)resetMove {
    ...
    [UIView animateWithDuration:ANIMATE_DURATION
                        ...
                        options:UIViewAnimationOptionBeginFromCurrentState
                     animations:^{
                         _mainVC.view.frame = CGRectMake(0, originY, self.view.frame.size.width, CGRectGetHeight(frame));
                     }
                     completion:^(BOOL finished) {
                         if (finished) {
                             [self resetMainView];
...
-(UIView *)getLeftView {
    ...
    if (_leftVC != nil) {
        ...
        _leftVC.view.frame = CGRectMake(0, originY, self.view.frame.size.width, CGRectGetHeight(frame));
...
-(UIView *)getRightView {
    ...
    if (_rightVC != nil) {
        ...
        _rightVC.view.frame = CGRectMake(SHOW_MAIN_WIDTH, originY, self.view.frame.size.width-SHOW_MAIN_WIDTH, CGRectGetHeight(frame));
...
-(void)showCenterViewWithShadow:(BOOL)value withOffset:(double)offset {
    if (value) {
        ...
        [_mainVC.view.layer setShadowColor:[UIColor blackColor].CGColor]; //将中间的分割线颜色改变
        [_mainVC.view.layer setShadowOpacity:0.5];
        [_mainVC.view.layer setShadowOffset:CGSizeMake(offset, offset)];
        
    } else {
        [_mainVC.view.layer setCornerRadius:0.0f];
        [_mainVC.view.layer setShadowOpacity:0];
        [_mainVC.view.layer setShadowOffset:CGSizeMake(offset, offset)];
    }
}
...
- (void) onBackButtonPressed {
    if(_isRightDrawerOpen || _isLeftDrawerOpen) {
        [self resetMove];
    }
